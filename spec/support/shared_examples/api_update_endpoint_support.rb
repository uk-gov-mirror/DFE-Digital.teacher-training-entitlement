...
RSpec.shared_examples "an API update endpoint" do
  ...
  let(:stub_service) do
    errors = instance_double(ActiveModel::Errors, blank?: true)
    service_double = instance_double(service, "#{action}": true, **service_args.merge(defined?(service_methods) ? service_methods : {}).merge(errors:))
